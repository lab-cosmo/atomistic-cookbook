import ase, ase.build, ase.optimize, ase.filters, ase.io
import numpy as np
import random
from metatomic.torch.ase_calculator import MetatomicCalculator

# create a fcc cell with 108 atoms, remove missing atom types
def mk_atoms(lattice_parameter=6.0, atomic_types=[]):
...
def mk_soup(model, n_soup=16):
    ...
    for i in range(n_soup):
        ...
        opt = ase.optimize.BFGS(ucf,trajectory=f'optimization.traj')
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Create a random soup of atoms and optimize them.")
    parser.add_argument("--model", type=str, required=True, help="Path to the metatomic model file.")
    parser.add_argument("--n_soup", type=int, default=16, help="Number of copies to create.")
