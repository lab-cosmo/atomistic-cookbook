...
2. **Training**: fine-tune the PET-MAD foundational MLIP on a 654-frame water dataset
...
# *shape* of the spectrum. This argument applies to the vibrational bands. For systems
# that carry a net ionic current (electrolytes, ionic liquids), the zero-frequency limit
# of the spectrum is proportional to the ionic conductivity, which *does* depend on the
# actual transported charge: in that case `only formal (oxidation-number) charges yield
# the correct DC limit, <https://doi.org/10.1038/s41567-019-0562-0>`_ even though they
# produce incorrect intensities. Here, neat water has zero ionic conductivity, so the
# choice of charge value is truly immaterial beyond an overall scale factor.
#
...
# function. In practice this is most easily computed via the `power spectral
# density <https://en.wikipedia.org/wiki/Spectral_density>`_ of the total cell dipole:
#
# .. math::
#
...
# :math:`1/6 = (1/3) \times (1/2)` combines the orientational average (isotropic liquid)
# with the cosine-transform convention. We compute the spectrum from a 5 ps production
# trajectory (after 1 ps of equilibration) at 300 K (`canonical
# <https://doi.org/10.1063/1.2408420>`_ NVT, CSVR thermostat, flexible TIP3P force
# field), giving a frequency resolution of ~7 cm⁻¹. The trajectory is generated by
# running LAMMPS with ``data/in_tip3p.lmp``:
...
# %%
# As a benchmark we use the experimental IR spectrum of liquid H₂O at 25 °C digitized
...
# one, it is worth spelling out the key symmetry constraint any dipole model must
# satisfy.
#
...
# time series needed for the IR spectrum. Because both heads read from the same
# descriptor, each training frame simultaneously constrains the potential-energy surface
# and the electronic-structure response---making joint training more data-efficient than
...
# vector.
#
# .. literalinclude:: options.yaml :language: yaml
#
# .. note::
#
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
# As discussed above, PET does not enforce rotation equivariance by construction---it
# *learns* equivariance from data augmentation. The residual error can be quantified
...
# reference. If the equivariance error is small compared to the model error, rotational
# inconsistency is not the bottleneck, and enforcing exact equivariance would not
# meaningfully improve accuracy. Note that the spread measures rotational
# *consistency*, not accuracy: it would be zero for an exactly equivariant model
# regardless of how far that model is from the DFT reference.
#
...
# model was trained on (energy, forces, dipole), producing a much larger file. We inject
# placeholder zeros---the values are never used.
...
# baseline earlier in the recipe---that one used the TIP3P trajectory, which gives
