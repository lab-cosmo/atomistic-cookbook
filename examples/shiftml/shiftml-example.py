...
if not os.path.exists(filename):
    url = f"https://archive.materialscloud.org/record/file?record_id=1503&filename=ShiftML_poly.zip"
...
# %%
fig, ax = plt.subplots(figsize=(4, 3))
ax.loglog(h_shieldings[2], np.abs(h_shieldings[0]-h_shieldings[1]), "o", markersize=2, alpha=0.5)
...
h_tensors = np.array(
    [ 
        ...
        for t,f  in zip(tensors, frames_cocaine) for iat in range(len(f))
    ]
)
...
chemiscope.show(
    ...
    shapes={
        "cs_ellipsoid":  {
            ...
            "parameters": { "global": {},
            "atom": [
                chemiscope.ellipsoid_from_tensor(cs_h*0.05)
                  for cs_h in h_tensors
            ]} }

        },
    #environments=[
    #    [i,h,3.5] for i in range(len(frames_cocaine)) for h in np.where(frames_cocaine[i].symbols=="H")[0]
    #],
    mode="structure",
    settings=chemiscope.quick_settings(periodic=True,
                                       structure_settings={"shape": ["cs_ellipsoid"],})
)